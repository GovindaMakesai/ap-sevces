...
const { buildGameSeedRows } = require('./gameCatalogSeed');

async function seedGameCatalog() {
  const rows = buildGameSeedRows();
  for (const g of rows) {
    await db.query(
      `INSERT INTO game_catalog (slug, name, emoji, html_path, category, min_bet, max_bet, sort_order, metadata)
  ...
  await db.query(
    `UPDATE game_catalog SET is_active = FALSE, updated_at = CURRENT_TIMESTAMP
     WHERE slug IN ('champ-car', 'champcar')`
  ).catch(() => {});
  return rows.length;
}
