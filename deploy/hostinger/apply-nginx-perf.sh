...
if [ -z "$DOMAIN" ]; then
  if [ -f "$SITE" ]; then
    DOMAIN="$(awk '/server_name/ && $2 !~ /_/ {print $2; exit}' "$SITE" | tr -d ';')"
  fi
fi
...
echo "==> Applying nginx perf config for $DOMAIN (no duplicate gzip)"
sed "s/__DOMAIN__/${DOMAIN}/g" "$APP_DIR/deploy/hostinger/nginx-ap-services.conf" > "$SITE"
nginx -t
systemctl reload nginx
echo "==> nginx reloaded OK"

echo "==> Ensure compression dep + restart API"
cd "$APP_DIR"
npm install compression --omit=dev --no-fund --no-audit 2>/dev/null || npm install compression --no-fund --no-audit
...
echo "  curl -sI https://${DOMAIN}/social-live.js | grep -i cache"
echo "Force-close the mobile app once so sessions/tokens pick up the new API."
