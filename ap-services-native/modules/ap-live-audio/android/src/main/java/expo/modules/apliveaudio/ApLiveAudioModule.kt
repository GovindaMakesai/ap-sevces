...
class ApLiveAudioModule : Module() {
    ...
    return try {
      am.getDevices(AudioManager.GET_DEVICES_OUTPUTS).any {
        it.type == AudioDeviceInfo.TYPE_WIRED_HEADSET ||
          ...
          (Build.VERSION.SDK_INT >= 26 && it.type == AudioDeviceInfo.TYPE_USB_DEVICE)
      }
    } catch (_: Throwable) {
      try {
        @Suppress("DEPRECATION")
  ...
  private fun routeToWiredHeadset(am: AudioManager, mode: String): Map<String, Any?> {
    requestMediaFocus(am)
    try {
      am.mode = AudioManager.MODE_NORMAL
    } catch (_: Throwable) {
   ...
   * Wired headphones: MODE_NORMAL, speaker off.
   * A2DP-only speakers: MODE_NORMAL. No headset: phone speaker.
   */
  private fun preferPlayback(): Map<String, Any?> {
    val am = audioManager()
    if (hasWiredHeadset(am) && !hasBluetooth(am)) {
      return routeToWiredHeadset(am, "playback")
    }
  ...
  private fun preferTalk(): Map<String, Any?> {
    val am = audioManager()
    if (hasWiredHeadset(am) && !hasBluetooth(am)) {
      return routeToWiredHeadset(am, "talk")
    }
    val bt = hasBluetooth(am)
    if (!bt) {
